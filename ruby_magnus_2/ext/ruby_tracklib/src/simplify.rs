...
struct SurfaceGroupIter<'a, 'b> {
    points: &'a [Point],
    mapping: std::cell::Ref<'b, SurfaceMapping>,
    group: Option<String>, // TODO: use a reference instead of cloning
}

impl<'a, 'b> SurfaceGroupIter<'a, 'b> {
    fn new(points: &'a [Point], mapping: std::cell::Ref<'b, SurfaceMapping>) -> Self {
        let group = points
            .first()
            .and_then(|point| mapping.get_surface_group(point))
            .cloned();
        Self { points, mapping, group }
    }
}
...
impl<'a, 'b> Iterator for SurfaceGroupIter<'a, 'b> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        for point in self.points.iter() {
            ...
            if self.group.as_ref() == group {
                partition_len += 1;
            } else {
                self.group = group.cloned();
                break;
            }
        }
...
pub(crate) fn simplify_points(
    points: &[Point],
    mapping: std::cell::Ref<SurfaceMapping>,
    tolerance: f64,
) -> HashSet<usize> {
...
mod tests {
    ...
    fn test_simplifying_zero_points() {
        ...
        assert_eq!(
            simplify_points(&[], RefCell::new(mapping).borrow(), 0.0),
            HashSet::new()
        );
    }
    ...
    fn test_simplifying_one_point() {
        let mapping = SurfaceMapping::new(0);
        assert_eq!(
            simplify_points(&[Point::default()], RefCell::new(mapping).borrow(), 0.0),
            HashSet::from_iter([0])
        );
    }
