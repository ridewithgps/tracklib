...
RSpec.describe Tracklib do
  context "simplify and encode" do
    it "can work in the simple case" do
      ...
      surface_mapping = Tracklib::SurfaceMapping.new(99)

      polyline = reader.section_simplified_polyline(
        0,
        surface_mapping,
        0.0,
        Tracklib::PolylineOptions.new([["y", 5],
                                       ["x", 5],
                                       ["S", 5, 99],
                                       ["R", 5, 99]])
      )
    ...
    it "can simplify and encode encrypted sections" do
      ...
      polyline = reader.section_simplified_polyline(
        ...
        surface_mapping,
        0.0,
        Tracklib::PolylineOptions.new([["y", 5], ["x", 5]]),
        key
      )
  ...
  context "simplify and serialize" do
    ...
    let(:surface_mapping) { Tracklib::SurfaceMapping.new(99) }
    ...
    it "works with no simplification" do
      ...
      expect(reader.section_data_simplified(
               0,
               surface_mapping,
               tolerance
             ))
        .to eq(expected)
      expect(reader.section_column_simplified(
               0,
               "x",
               surface_mapping,
               tolerance
