...
RSpec.describe Tracklib do
  ...
  it "raises errors for invalid F64 scale" do
    expect { Tracklib::Schema.new([["a", :f64]]) }.to raise_error(TypeError, "no implicit conversion of nil into Integer")
    expect { Tracklib::Schema.new([["a", :f64, "2"]]) }.to raise_error(TypeError, "no implicit conversion of String into Integer")
    expect { Tracklib::Schema.new([["a", :f64, 500]]) }.to raise_error(RangeError, "fixnum too big to convert into `u8`")
  end
  ...
  it "can write metadata" do
    ...
    expect { Tracklib.write_track(["Foo"], []) }.to raise_error(TypeError, "no implicit conversion of String into Array")
    expect { Tracklib.write_track([[:created_at, "foo"]], []) }.to raise_error(TypeError, "no implicit conversion of String into Time")
    expect { Tracklib.write_track([[:track_type, :foo, 5]], []) }.to raise_error(ArgumentError, "Metadata Entry Track Type 'foo' unknown")
    expect { Tracklib.write_track([[:track_type, :route, "foo"]], []) }.to raise_error(TypeError, "no implicit conversion of String into Integer")
  end
