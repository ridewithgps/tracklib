...
RSpec.describe Tracklib do
  context "polylines" do
    it "can work in the simple case" do
      ...
      polyline = reader.section_polyline(0, Tracklib::PolylineOptions.new([["y", 5], ["x", 5]]))
      expect(decode_polyline(polyline, [5, 5]))
        .to eq([12.0, 40.0, 800.0, 41.0])

      # Parameters in the opposite order
      polyline = reader.section_polyline(0, Tracklib::PolylineOptions.new([["x", 5], ["y", 5]]))
      expect(decode_polyline(polyline, [5, 5]))
    ...
    it "can encode at different precisions" do
      ...
      polyline = reader.section_polyline(0, Tracklib::PolylineOptions.new([["y", 1], ["x", 5]]))
      expect(decode_polyline(polyline, [1, 5]))
        .to eq([12.0, 40.0, 800.0, 41.0])

      # Parameters in the opposite order
      polyline = reader.section_polyline(0, Tracklib::PolylineOptions.new([["x", 5], ["y", 1]]))
    ...
    it "can handle default values" do
      ...
      polyline = reader.section_polyline(0, Tracklib::PolylineOptions.new([["y", 5], ["x", 5], ["S", 5, 99]]))
    ...
    it "can skip encoding if a field is missing from the track" do
      ...
      polyline = reader.section_polyline(0, Tracklib::PolylineOptions.new([["y", 5], ["x", 5], ["e", 5]]))
    ...
    it "can encode encrypted sections" do
      ...
      polyline = reader.section_polyline(0, Tracklib::PolylineOptions.new([["y", 5], ["x", 5]]), key)
      expect(decode_polyline(polyline, [5, 5]))
        .to eq([12.0, 40.0, 800.0, 41.0])
    end
    ...
    it "can compute the 'd' field" do
      ...
      polyline = reader.section_polyline(0, Tracklib::PolylineOptions.new([["y", 5], ["x", 5], ["d", 5]]))
      expect(decode_polyline(polyline, [5, 5, 5]))
        .to eq([72.1, -122.402, 0.0, 72.309, -122.5, 23_477.14945])
    end
  end
end
